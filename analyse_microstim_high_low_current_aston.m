function analyse_microstim_high_low_current(date,allInstanceInd)
%7/2/18
%Written by Xing, calculates behavioural performance during a
%microstimulation 2-phosphene task, with two conditions: using current
%levels of 1.5 or 2.5 times the current threshold. Furthermore, a minimum
%current level of 20 or 30 uA is imposed.
...
if processRaw==1
    for instanceCount=1%:length(allInstanceInd)
        ...
        meanPerfMicrostimCond=[];
        numTrialsMicrostimCond=[];
        for stimPatternInd=1:2%analyse performance for each condition
            for targetLocInd=1:2
                for currentCondInd=1:2                    
                    stimPatternTrialsInd=find(allStimPattern==stimPatternInd);%identify trials with this particular stimulation condition
                    targetLocTrialsInd=find(allTargetLocation==targetLocInd);
                    currentCondTrialsInd=find(allCondCurrent==currentCondInd);
                    tempCorrect1=intersect(correctMicrostimTrialsInd,stimPatternTrialsInd);%trialNo for microstim trials with a correct saccade for that condition
                    tempCorrect2=intersect(tempCorrect1,targetLocTrialsInd);%trialNo for microstim trials with a correct saccade for that condition
                    correctMicrostimTrialsStimCond=intersect(tempCorrect2,currentCondTrialsInd);%trialNo for microstim trials with a correct saccade for that condition
                    tempCorrect1=intersect(incorrectMicrostimTrialsInd,stimPatternTrialsInd);%trialNo for microstim trials with an incorrect saccade for that condition
                    tempCorrect2=intersect(tempCorrect1,targetLocTrialsInd);%trialNo for microstim trials with an incorrect saccade for that condition
                    incorrectMicrostimTrialsStimCondInd=intersect(tempCorrect2,currentCondTrialsInd);%trialNo for microstim trials with an incorrect saccade for that condition
                    meanPerfMicrostimCond=[meanPerfMicrostimCond length(correctMicrostimTrialsStimCond)/(length(correctMicrostimTrialsStimCond)+length(incorrectMicrostimTrialsStimCondInd))];%mean performance for that condition
                    numTrialsMicrostimCond=[numTrialsMicrostimCond length(correctMicrostimTrialsStimCond)+length(incorrectMicrostimTrialsStimCondInd)];%mean performance for that condition
                end
            end
        end
        ...
        if analyseConds==1            
            ...
            condDiagram=imread('D:\data\high_low_currents_diagram.PNG');
            image(condDiagram);
            set(gca,'XTickLabel',[]);
            set(gca,'YTickLabel',[]);
            ylim([0 720]);
            ...
            for electrodeCount=1:size(setElectrodes,2)
                for setInd=1:2
                    ...
                    currentAmplitudes(setInd,electrodeCount)=currentThreshold;
                    if electrodeCount==1
            ...
            if min30==1
                condPair=[];%check whether same current levels delivered, between conditions where current delivered was multiple of 1.5 or 2.5 times threshold, as this sometimes happens when a minimum current level is imposed
                if currentAmplitudes(1,1)*1.5<=30&&currentAmplitudes(1,4)*1.5<=30&&currentAmplitudes(1,1)*2.5<=30&&currentAmplitudes(1,4)*2.5<=30
                    condPair(1)=0;%comparison of stim conds 1 & 2
                else
                    condPair(1)=1;
                end
                if currentAmplitudes(2,1)*1.5<=30&&currentAmplitudes(2,4)*1.5<=30&&currentAmplitudes(2,1)*2.5<=30&&currentAmplitudes(2,4)*2.5<=30
                    condPair(2)=0;%comparison of stim conds 3 & 4
                else
                    condPair(2)=1;
                end
                if currentAmplitudes(1,2)*1.5<=30&&currentAmplitudes(1,3)*1.5<=30&&currentAmplitudes(1,2)*2.5<=30&&currentAmplitudes(1,3)*2.5<=30
                    condPair(3)=0;%comparison of stim conds 5 & 6
                else
                    condPair(3)=1;
                end
                if currentAmplitudes(2,2)*1.5<=30&&currentAmplitudes(2,3)*1.5<=30&&currentAmplitudes(2,2)*2.5<=30&&currentAmplitudes(2,3)*2.5<=30
                    condPair(4)=0;%comparison of stim conds 7 & 8
                else
                    condPair(4)=1;
                end
            elseif min30==0
            ...
            for stimPatternInd=1:length(meanPerfMicrostimCond)
...
%             print(pathname,'-dtiff');
        ...
        save([rootdir,'\',date,'\','\perf_conds_high_vs_low_',date,'.mat'],'meanPerfMicrostimCond','numTrialsMicrostimCond','currentAmplitudes','condPair');
...
%         ylabel('performance');
        pathname=fullfile(rootdir,date,['behavioural_performance_high_vs_low_currents_',date]);
        set(gcf,'PaperPositionMode','auto','Position',get(0,'Screensize'))
        print(pathname,'-dtiff');
    end
end
