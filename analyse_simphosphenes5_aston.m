function analyse_simphosphenes5(date,allInstanceInd,allGoodChannels)
%3/7/17
%Written by Xing. Loads and analyses MUA data, during presentation
%of simulated phosphene letters, for V1 and V4 channels.
...
V1Chs=[1:32 97:128 161:224 257:1024];
V4Chs=[33:96 129:160 225:256];
allChs=1:1024;
for areaInd=1:3
    switch areaInd
        case 1
            chsOfInterest=V1Chs;%V1
            area='V1';
        case 2
            chsOfInterest=V4Chs;%V4
            area='V4';
        case 3
            chsOfInterest=allChs;%V1 & V4
            area='V1_V4';
    end
    allNormMeanChannelResponse1024=allNormMeanChannelResponse1024(chsOfInterest,:);
    allChannelRFs=allChannelRFs(chsOfInterest,:);
    % figure;hold on
    for letterCond=7:10
        figure;hold on
        ...
        if letterCond<10%all symbols except the square
            ...
            pathname=fullfile('D:\data',date,[area,' visual responses to simulated phosphene letter ',allLetters(letterCond)]);
        ...
        pathname=fullfile('D:\data',date,[area,' responses to letter ',allLetters(letterCond),'.mat']);
        if letterCond==10%square full of sim phosphenes
            pathname=fullfile('D:\data',date,[area,' responses to square.mat']);
        end
        save(pathname,'framesResponse','-v7.3')
        %     movieFig=figure;
        %     movie(movieFig,framesResponse,1,50);
        moviename=fullfile('D:\data',date,[area,' responses to letter ',allLetters(letterCond),'.avi']);
        if letterCond==10%square full of sim phosphenes
            moviename=fullfile('D:\data',date,[area,' responses to square.avi']);
        end
    ...
    for letterCond=9%1:10
        if letterCond<10
            load(['D:\data\110717_B1_B2_120717_B123\',area,' responses to letter ',allLetters(letterCond),'.mat'])
        else
            load('D:\data\110717_B1_B2_120717_B123\',area,' responses to square.mat')
        end
        figure;
        imshow(framesResponse(timePoint).cdata)
        if letterCond<10
            pathname=fullfile('D:\data',date,[area,' visual responses to simulated phosphene letter ',allLetters(letterCond),'_time',num2str(timePoint)]);
        else
            pathname=fullfile('D:\data',date,[area,' visual responses to simulated phosphene square_time',num2str(timePoint)]);
        end
        print(pathname,'-dtiff');
    end
    ...
    pathname=fullfile('D:\data',date,[area,' visual responses to simulated phosphene letters_time',num2str(timePoint)]);
    print(pathname,'-dtiff');
end